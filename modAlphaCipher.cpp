...
inline std::wstring modAlphaCipher::getValidKey(const std::wstring & s)
{
    ...
    std::wstring tmp(s);
    for (auto & c:tmp) {
        if (!isalpha(c))            throw cipher_error(std::string("Invalid key "));
...
inline std::wstring modAlphaCipher::getValidOpenText(const std::wstring & s)
{
    ...
    for (auto c:s) {
        if (isalpha(c)) {
            if (islower(c))                tmp.push_back(toupper(c));
            else                tmp.push_back(c);
        }
    }
    if (tmp.empty())        throw cipher_error("Empty open text");
    return tmp;
}
inline std::wstring modAlphaCipher::getValidCipherText(const std::wstring & s)
{
    if (s.empty())        throw cipher_error("Empty cipher text");
    for (auto c:s) {
        if (!isupper(c))            throw cipher_error(std::string("Invalid cipher text "));
    }
    return s;
}
